...
public class AlipayPagePayController {
    ...
    private IRoleServie productService;
    @Resource
    IdWorker idWorker;
    ...
    @PostMapping("/create")
    public Response create(@RequestBody String userId,Role role){
        MemberDetails memberDetails=new MemberDetails();
        memberDetails.setOrderNum(idWorker.nextId()+"");
        memberDetails.setSubject(role.getRoleName());
        memberDetails.setOrderAmount(role.getRolePrice());
        memberDetails.setCreateTime(new Date());
        memberDetails.setOrderStatus(Integer.valueOf(OrderStatusEnum.WAIT_PAY.key));//支付状态
        return Response.success().setData(ordersService.saveOrder(memberDetails));
